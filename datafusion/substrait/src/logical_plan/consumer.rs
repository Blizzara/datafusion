...
fn from_substrait_literal(
    ...
    let scalar_value = match &lit.literal_type {
        ...
        Some(LiteralType::List(l)) => {
            ...
            let elements = l
                ...
                .map(|el| {
                    ...
                    from_substrait_literal(el, dfs_names, &mut element_name_idx)
                })
                .collect::<Result<Vec<_>>>()?;
        ...
        Some(LiteralType::EmptyMap(m)) => {
            ...
            let key_type = from_substrait_type(key, dfs_names, name_idx)?;
            let value_type = from_substrait_type(value, dfs_names, name_idx)?;
    ...
    if let Some(kind) = &null_type.kind {
        match kind {
            ...
            r#type::Kind::Map(map) => {
                ...
                let key_type = from_substrait_type(key_type, dfs_names, name_idx)?;
                let value_type = from_substrait_type(value_type, dfs_names, name_idx)?;
